...
/**
 * @author Ataraxia
 * @create 2022/4/5 12:40
 * @description 堆优化的Dijkstra + 链式前向星
 */
public class _dijkstra堆优化 {
    ...
    public static int priorityDijkstra(int s) {
        ...
        while (!q.isEmpty()) {
            ...
            // 如果节点no已存在最小路径，就continue
            if (vis[no]) {
                continue;
            }

            // 因为节点no已经是离起点最近的节点，所以标记为已找到最小路径
            vis[no] = true; // 出队判重

            // 遍历这个点的所有通路，即所有未找到最小路径的节点
            for (int i = head[no]; i != 0; i = e[i].next) {
                int v = e[i].to;
                // 如果通过节点no到节点i比直接从起点到节点v距离更短，就进行松弛;
